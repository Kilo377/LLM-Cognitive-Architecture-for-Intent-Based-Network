...
carrierFrequency = 2.646e9; % In hertz
...
gNBTxSite = txsite(Name="gNB",TransmitterFrequency=carrierFrequency, ...
...
% Create a gNB node and specify its position, channel bandwidth, receive gain,
% number of transmit and receive antennas, carrier frequency, and SRS
% transmission periodicity (in slots) for all connecting UE nodes
gNB = nrGNB(Position=gNBPosition,ChannelBandwidth=10e6,ReceiveGain=12, ...
    NumTransmitAntennas=16,NumReceiveAntennas=8, ...
    CarrierFrequency=gNBTxSite.TransmitterFrequency,SRSPeriodicityUE=40);
...
ueTxSites = txsite(Name="UE",Latitude=ueLat,Longitude=ueLon(idx), ...
    AntennaHeight=1,TransmitterFrequency=carrierFrequency);
...
connectUE(gNB,UEs,RLCBearerConfig=nrRLCBearerConfig(BucketSizeDuration=10),CSIReportPeriodicity=80)
