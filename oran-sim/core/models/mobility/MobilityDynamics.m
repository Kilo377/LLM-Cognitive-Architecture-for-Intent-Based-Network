classdef MobilityDynamics < handle
    %MOBILITYDYNAMICS Mixed periodic + step + noise dynamics for mobility.
    ...
    methods
        function obj = MobilityDynamics(dynamicCfg)
            if nargin < 1
                dynamicCfg = struct();
            end
        ...
        function out = get(obj, slotNow)
            ...
            out = struct();
            out.speedScale = obj.clamp(1 + obj.amplitude * mix1, 0.4, 2.0);

            baseJitter = 0.15;
            out.directionJitter = obj.clamp(baseJitter * (1 + obj.amplitude * mix2), 0, 0.6);

            basePauseProb = 0.02;
            out.pauseProbability = obj.clamp(basePauseProb * (1 + obj.amplitude * mix3), 0, 0.2);
        end
    end
    ...
    methods (Access=private)
        ...
        function out = defaultOut(~)
            out = struct();
            out.speedScale = 1.0;
            out.directionJitter = 0.15;
            out.pauseProbability = 0.0;
        end
