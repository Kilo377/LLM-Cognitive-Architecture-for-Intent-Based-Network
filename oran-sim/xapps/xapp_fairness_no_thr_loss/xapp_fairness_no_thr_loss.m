function action = xapp_fairness_no_thr_loss(input)
    ...
    if ~state.baselineLoaded
        state.baselineThr = readBaselineValue('throughput_Mbps');
        state.baselineLoaded = true;
    end

    thr = getField(obs, "kpi.capacity.throughput_Mbps_total", nan);
    state = updateBuffer(state, thr);

    if slot >= state.nextUpdate && state.sampleCount >= state.window
        winThr = mean(state.buf, 'omitnan');

        gap = max(0, (state.baselineThr - winThr) / state.baselineThr);
        scale = max(0, 1 - gap / state.thrDropCap);
        state.weightUE = computeFairWeights(obs, state.weightAmp, scale);
        if scale <= 0
        ...
        if state.debugEnable
            fprintf('[DEBUG][slot=%d][xapp_fairness_no_thr_loss] thrW=%.2f base=%.2f gap=%.3f scale=%.3f decision=%s\n', ...
                slot, winThr, state.baselineThr, gap, scale, state.lastDecision);
        end
    end
...
function w = computeFairWeights(obs, weightAmp, scale)
...
function v = readBaselineValue(metricName)
    ...
    if ~isfile(csvPath)
        error('xapp_fairness_no_thr_loss:MissingBaseline', ...
    ...
    if isempty(idxMetric) || isempty(idxValue)
        error('xapp_fairness_no_thr_loss:BaselineFormat', ...
    ...
    vals = tbl{:, idxValue};
    if isempty(metrics)
        error('xapp_fairness_no_thr_loss:BaselineEmpty', 'baseline.csv is empty');
    end

    idx = find(lower(metrics) == lower(string(metricName)), 1);
    if isempty(idx)
        error('xapp_fairness_no_thr_loss:BaselineMissingMetric', ...
            'baseline.csv missing metric %s', metricName);
    end

    v = vals(idx);
    if isempty(v) || ~isnumeric(v) || ~isscalar(v) || ~isfinite(v) || v <= 0
        error('xapp_fairness_no_thr_loss:BaselineInvalidValue', ...
