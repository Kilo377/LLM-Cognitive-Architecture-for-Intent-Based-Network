function action = xapp_throughput_maximizer(input)
    ...
    prbUtil = getField(obs, "kpi.instant.prbUtilMean", 0);
    meanSinr = getField(obs, "kpi.instant.meanSINR_dB", 0);
    ...
    if slot >= state.nextUpdate && state.sampleCount >= state.window
        ...
        if state.debugEnable
            fprintf('[DEBUG][slot=%d][xapp_throughput_maximizer] action=%d reward=%.4f thr=%.2f drop=%.4f bler=%.4f\n', ...
    ...
    [bwScale, txOffset] = radioControl(state, prbUtil, meanSinr, dropRatio, meanBler);
    action.radio.bandwidthScale = bwScale * ones(numCell,1);
    action.radio.txPowerOffset_dB = txOffset * ones(numCell,1);
...
function state = initState()
    ...
    state.prbUtilHigh = 0.9;
    state.bandwidthScaleMax = 1.5;
    state.sinrLow_dB = 25.0;
    state.txPowerOffsetMax_dB = 6.0;
...
function [bwScale, txOffset] = radioControl(state, prbUtil, meanSinr, dropRatio, meanBler)
    bwScale = 1.0;
    txOffset = 0.0;

    if isnan(state.baselineDrop) || isnan(state.baselineBler)
        return;
    end
    ...
    if dropRatio > dropCap || meanBler > blerCap
        return;
    end

    if prbUtil > state.prbUtilHigh
        utilRatio = min(max((prbUtil - state.prbUtilHigh) / max(1 - state.prbUtilHigh, eps), 0), 1);
        bwScale = 1.0 + utilRatio * (state.bandwidthScaleMax - 1.0);
    end

    if meanSinr < state.sinrLow_dB
        sinrGap = min(max((state.sinrLow_dB - meanSinr) / max(state.sinrLow_dB, 1), 0), 1);
        txOffset = sinrGap * state.txPowerOffsetMax_dB;
    end
end
